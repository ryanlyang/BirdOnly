...
mkdir -p "${ROOT}/run_logs" "${ROOT}/preflight" \
  "${ROOT}/submission_receipts" "${ROOT}/background_sanitized" \
  "${ROOT}/fusion_sanitized"
...
if [[ ! -f "${BANK}/sanitized_mask_bank_receipt.json" ]]; then
  echo "Rejected sanitized bank is missing: $BANK" >&2
  exit 2
fi
if ! grep -q '"status": "rejected"' \
  "${BANK}/sanitized_mask_bank_receipt.json"; then
  echo "Diagnostic resume requires the immutable rejected bank: $BANK" >&2
  exit 2
fi
if [[ -e "$SANITIZED" || -e "$FUSION" ]]; then
  echo "Refusing duplicate diagnostic Phase 3 output" >&2
  exit 2
fi
if [[ -n "$(squeue -h -u "$USER" -n setv_sanitized_smoke,setv_sanitized_train,setv_sanitized_fusion 2>/dev/null)" ]]; then
  echo "Refusing duplicate diagnostic Phase 3 submission: active jobs exist" >&2
  exit 2
fi

preflight_report="${ROOT}/preflight/phase3_diagnostic_resume_$(date -u +%Y%m%dT%H%M%SZ)_${BASHPID}.json"
...
SETV_ALLOW_REJECTED_SANITIZED_BANK=1
export SETV_REPO SETV_OBJECT_SEED SETV_MASK_SEED SETV_SANITIZED_SEED
export SETV_SANITIZED_FUSION_SEED SETV_ALLOW_REJECTED_SANITIZED_BANK
...
receipt="${ROOT}/submission_receipts/phase3_sanitized_diagnostic_${fusion_id}.txt"
{
  echo "submitted_at=$(date --iso-8601=seconds)"
  echo "scientific_override=rejected_sanitized_bank_diagnostic_pilot"
  echo "source_audit_job_id=21917"
  echo "leakage_gate_accepted=false"
  echo "sanitization_claim_eligible=false"
  ...
  echo "reused_rejected_bank=$BANK"
  echo "rejected_bank_receipt_sha256=$(sha256sum "${BANK}/sanitized_mask_bank_receipt.json" | awk '{print $1}')"
  echo "rejected_bank_manifest_sha256=$(sha256sum "${BANK}/artifact_manifest.json" | awk '{print $1}')"
  echo "leakage_audit_sha256=$(sha256sum "${BANK}/leakage_audit.json" | awk '{print $1}')"
...
echo "Submitted diagnostic Phase 3: smoke=$smoke_id train=$train_id fusion=$fusion_id"
echo "The leakage gate remains failed; outputs are diagnostic, not sanitization-compliant."
